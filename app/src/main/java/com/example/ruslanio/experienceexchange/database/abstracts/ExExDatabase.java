...
@Database(entities = {Interest.class, User.class}, version = 1)
public abstract class ExExDatabase extends RoomDatabase {
    ...
    static synchronized ExExDatabase getInstance(Context context){
        if (mInstance == null)
    ...
    private static ExExDatabase create(Context context) {
        return Room.databaseBuilder(context, ExExDatabase.class, DB_NAME).build();
    }
